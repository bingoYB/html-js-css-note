...
比如我定义了两个对象（`Person`和`Student`），Person里有两个属性`name`和`getName`,Student也需要这同样的属性，但又不想重复的写这部分代码，这样就有一个需求，Student需要从Person里继承这两个属性。
...
在大多数浏览器中实现了通过`__proto__`属性来查看原型（ie不支持），并可通过这个属性进行设置。
